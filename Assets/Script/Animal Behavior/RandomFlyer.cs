...
{	
    ...
    void FixedUpdate()
    {
        ...
        if (body.transform.position.y < currentTarget.position.y + yMinMax.x + 10f ||
            body.transform.position.y > currentTarget.position.y + yMinMax.y -10f)
        {
            if (body.transform.position.y < currentTarget.position.y + yMinMax.x + 10f) rotateTarget.y = 1f; else rotateTarget.y = -1f;
        }
    ...
    private Vector3 ChangeDirection(Vector3 currentPosition)
    {
        ...
        {	
            ...
            randomizedBase = currentTarget.position;
